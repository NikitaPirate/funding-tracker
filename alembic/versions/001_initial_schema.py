...
Revision ID: 001
Revises:
Create Date: 2025-11-13 14:26:19.594490
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_funding_rate_record_contract_id_timestamp",
        "funding_rate_record",
        ["contract_id", "timestamp"],
        unique=False,
    )
    op.create_table(
        "unsettled_funding_rate_record",
        sa.Column("timestamp", sa.DateTime(), nullable=False),
        sa.Column("contract_id", sa.Uuid(), nullable=False),
        sa.Column("funding_rate", sa.Float(), nullable=False),
        sa.ForeignKeyConstraint(
    ...
    with op.get_context().autocommit_block():
        op.execute(
            "SELECT create_hypertable('funding_rate_record', 'timestamp', "
            "chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE)"
        )
        op.execute(
            "SELECT create_hypertable('unsettled_funding_rate_record', 'timestamp', "
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table("unsettled_funding_rate_record")
    op.drop_index("ix_funding_rate_record_contract_id_timestamp", table_name="funding_rate_record")
    op.drop_table("funding_rate_record")
